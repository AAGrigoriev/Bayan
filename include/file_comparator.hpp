...
class file_comparator : public IFile_comparator {
 public:
  ...
  void add_paths(const group_path& size_paths) override{
    ...
    duplicates_.erase(std::remove_if(duplicates_.begin(), duplicates_.end(), [](const auto& path_vector) { return path_vector.size() == 1;}), duplicates_.end());
  }

  const parsing_result& duplicates() const override {
      return duplicates_;
  }
