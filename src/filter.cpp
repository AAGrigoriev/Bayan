...
namespace bayan
{
    filter_dir::filter_dir(opt_filter&& opt_filter): filter_opt_(std::move(opt_filter)) {
    ...
    bool filter_dir::approach_dir(const fs::path &path) {
        return std::any_of(filter_opt_.exclude_path.begin(), filter_opt_.exclude_path.end(),
    ...
    bool filter_dir::approach_file(const fs::path &path) {
        ...
        {
            return std::any_of(filter_mask_.begin(), filter_mask_.end(),
                               [&path](mask& mask_)
                               {
                                    return mask_.isValid(path.string());
                               }
                               );
        }
            return false;
