...
//         options.get().excludePaths, options.get().levelScannig,
//         options.get().masks, options.get().minFileSize);
//     auto groupPath = fileScanner.Scan(options.get().includePaths);

//     Otus::Duplicatescanner dupScanner(options.get().blockSize,
//                                       options.get().hash);
...
//   ASSERT_EQ(result.size(), (std::size_t)1);
//   ASSERT_EQ(result[0].size(), (std::size_t)2);
//   ASSERT_THAT(result[0], ElementsAre(StrEq("file2.txt"),
//   StrEq("file1.txt")));
// }
...
//   auto result =
//       Run({"test_bayan", "-idata/test_2/", "-edata/test_2/level_1/", "-l1"});
//   ASSERT_EQ(result.size(), (std::size_t)1);
//   ASSERT_EQ(result[0].size(), (std::size_t)2);
//   ASSERT_THAT(result[0],
//               ElementsAre(StrEq("hello_copy.cpp"), StrEq("hello.cpp")));
...
//   // EXPECT_THAT(result[0], ElementsAre( StrEq("file3.txt"),
//   StrEq("file1.txt"),
//   // StrEq("file2.txt") ));
// }
