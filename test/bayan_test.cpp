...
//   auto result = Run({"bayan", "-idata/test_1/", "-s100", "-m*.cpp",
//   "-m*.txt"}); ASSERT_EQ(result.size(), (std::size_t)1);
//   ASSERT_EQ(result[0].size(), (std::size_t)2);
//   ASSERT_THAT(result[0],
//               ElementsAre(StrEq("hello_copy.cpp"), StrEq("hello.cpp")));
// }
...
//   auto result = Run({"bayan", "-idata/test_2/", "-edata/test_2/level_1/",
//   "-r1",
//                      "-m*.cpp", "-m*.txt"});
//   ASSERT_EQ(result.size(), (std::size_t)1);
//   ASSERT_EQ(result[0].size(), (std::size_t)2);
...
//   auto result = Run({"bayan", "-idata/test_3/", "-r1", "-m*.cpp",
//   "-m*.txt"}); ASSERT_EQ(result.size(), (std::size_t)1);
